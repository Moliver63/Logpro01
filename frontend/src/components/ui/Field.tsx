  ...
  return (
    <label className="block">
      <span className="block font-body text-[13px] font-medium tracking-wide text-tintaSuave uppercase">
        {label}
        {required && <span className="ml-1 text-alerta">*</span>}
      </span>
...
const inputBase =
  "w-full rounded-card border border-ledgerLine bg-white/60 px-3 py-2 font-mono text-[15px] text-tinta " +
  "placeholder:text-tintaSuave/60 focus:border-soja focus:bg-white focus:outline-none transition-colors";
...
export function SelectField(
  ...
  return (
    <select {...rest} className={`${inputBase} appearance-none bg-white/60 ${className ?? ""}`}>
      {children}
    </select>
  );
}
...
export function Card({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={`rounded-card border border-ledgerLine bg-[#F8F6EE] ${className ?? ""}`}>{children}</div>
  );
}
